...
<body>
	<div class="container">
		...
		<nav class="top-navbar">
			...
			<div class="nav-links">
				...
				<% } %>
				<a href="account.jsp" class="nav-button active">Account
					Management</a> <a href="profile.jsp" class="nav-button user-profile">
		...
		<div class="storage-selection">
			...
			<div class="storage-options">
				<label class="storage-option"> <input type="radio"
					name="storageType" value="database" checked> <span>MySQL
						Database</span>
				</label> <label class="storage-option"> <input type="radio"
					name="storageType" value="mongodb"> <span>MongoDB</span>
				</label> <label class="storage-option"> <input type="radio"
					name="storageType" value="collection"> <span>In-Memory
						Collection</span>
				</label>
			</div>
		</div>
		...
		<!-- Operation Buttons - Role-based -->
		<div class="operation-buttons">
			<% if ("MANAGER".equals(session.getAttribute("role")) || "EMPLOYEE".equals(session.getAttribute("role")) ||"CUSTOMER".equals(session.getAttribute("role")) ) { %>
			<button data-section="create-account" class="operation-btn">Create
				Account</button>
			<button data-section="joint-accounts" class="operation-btn">Joint
				Accounts</button>
			<% } %>
			<% if ("CUSTOMER".equals(session.getAttribute("role"))) { %>
			<button data-section="transactions" class="operation-btn">My
				Transactions</button>
			<% } %>
			<button data-section="view-accounts" class="operation-btn">View
				Accounts</button>
			<button data-section="transaction-history" class="operation-btn">Transaction
				History</button>
		</div>
		...
		<div class="success-message">
			...
			<p>
				...
				<%= request.getAttribute("newAccountNo") %></p>
				...
				<div class="form-row">
					...
					<div class="form-group">
						<label for="balance">Initial Balance:</label> <input type="number"
							id="balance" name="balance" step="0.01" min="100" required>
					</div>
				</div>
		...
		<!-- Transactions Section (Customers Only) -->
		<% if ("CUSTOMER".equals(session.getAttribute("role"))) { %>
		<div id="transactions" class="content-section">
			...
			<div id="deposit" class="tab-content active">
				...
				<form id="depositForm" action="account" method="post"
					...
					<input type="hidden" name="action" value="deposit"> <input
						type="hidden" name="storageType" value="database"> <input
						type="hidden" name="csrfToken"
						value="<%= session.getAttribute("csrfToken") %>">

					<div class="form-row">
						<div class="form-group">
							<label for="depositAccountNo">Select Account:</label> <select
								id="depositAccountNo" name="accountNo" required>
						...
						<div class="form-group">
							<label for="depositAmount">Amount:</label> <input type="number"
								id="depositAmount" name="amount" step="0.01" min="1" required>
						</div>
					</div>
			...
			<div id="withdraw" class="tab-content">
				...
				<form id="withdrawForm" action="account" method="post"
					...
					<div class="form-row">
						<div class="form-group">
							<label for="withdrawAccountNo">Select Account:</label> <select
								id="withdrawAccountNo" name="accountNo" required>
								<option value="">-- Select Your Account --</option>
							</select>
						...
						<div class="form-group">
							<label for="withdrawAmount">Amount:</label> <input type="number"
								id="withdrawAmount" name="amount" step="0.01" min="1" required>
						</div>
					</div>
		...
		<div id="transaction-history" class="content-section">
			...
			<form id="transactionHistoryForm" class="form-container">
				<div class="form-row">
					<div class="form-group">
						<label for="historyAccountNo">Account Number:</label> <input
							type="number" id="historyAccountNo" name="accountNo" required>
					</div>
					<div class="form-group">
						<label for="historyLimit">Number of Records:</label> <select
							id="historyLimit" name="limit">
				...
				<button type="submit" class="submit-btn">Get Transaction
					History</button>
			</form>
		...
		<div id="view-accounts" class="content-section">
				...
				<form id="viewAccountForm" action="account" method="get"
					...
					<div class="form-group">
						<label for="accountNo">Account Number:</label> <input
							type="number" id="accountNo" name="accountNo" required>
					</div>
			...
			<!-- Customer View (Staff Only) -->
			<div id="customer" class="tab-content">
				<h4>View Accounts by Customer</h4>
				<form id="viewAccountsByCustomerForm" action="account" method="get"
					class="form-container">
					<input type="hidden" name="action" value="getByCustomer"> <input
						type="hidden" name="storageType" value="database">

					<div class="form-group">
						<label for="customerIdFilter">Customer ID:</label> <input
							type="number" id="customerIdFilter" name="customerId" required>
					</div>

					<button type="submit" class="submit-btn">View Customer
						Accounts</button>
				</form>
			</div>
			<% } %>
		...
		<!-- Joint Accounts Section (Staff Only) -->
		<% if ("MANAGER".equals(session.getAttribute("role")) || "EMPLOYEE".equals(session.getAttribute("role"))||"CUSTOMER".equals(session.getAttribute("role"))) { %>
		<div id="joint-accounts" class="content-section">
			...
			<div id="create" class="tab-content active">
				...
				<form id="createJointAccountForm" action="account" method="post"
					class="form-container">
					<input type="hidden" name="action" value="createJoint"> <input
						type="hidden" name="csrfToken"
						value="<%= session.getAttribute("csrfToken") %>"> <input
						type="hidden" name="storageType" value="database">

					<div class="form-group">
						<label for="customerIds">Customer IDs (comma-separated):</label> <input
							type="text" id="customerIds" name="customerIds"
							pattern="\d+(,\s*\d+)+"
					...
					<div class="form-row">
						<div class="form-group">
							<label for="jointBalance">Initial Balance:</label> <input
								type="number" id="jointBalance" name="balance" step="0.01"
								min="100" required>
						</div>
						<div class="form-group">
							<label for="jointBranchId">Branch:</label> <select
								id="jointBranchId" name="branchId" required>
								<option value="">Select Branch</option>
								<option value="1">Main Branch</option>
								<option value="2">Anna Nagar Branch</option>
								<option value="3">Adyar Branch</option>
					...
					<button type="submit" class="submit-btn">Create Joint
						Account</button>
				</form>
			</div>
			...
			<div id="add" class="tab-content">
				...
				<form id="addJointHolderForm" action="account" method="post"
					...
					<div class="form-row">
						<div class="form-group">
							<label for="accountNoToAdd">Account Number:</label> <input
								type="number" id="accountNoToAdd" name="accountNo" required>
						</div>
						<div class="form-group">
							<label for="customerIdToAdd">Customer ID:</label> <input
								type="number" id="customerIdToAdd" name="customerId" required>
						</div>
					</div>
			...
			<div id="remove" class="tab-content">
				...
				<form id="removeJointHolderForm" action="account" method="post"
					...
					<div class="form-row">
						<div class="form-group">
							<label for="accountNoToRemove">Account Number:</label> <input
								type="number" id="accountNoToRemove" name="accountNo" required>
						</div>
						<div class="form-group">
							<label for="customerIdToRemove">Customer ID:</label> <input
								type="number" id="customerIdToRemove" name="customerId" required>
						</div>
					</div>

					<button type="submit" class="submit-btn danger">Remove
						Joint Holder</button>
				</form>
			</div>
			...
			<div id="view" class="tab-content">
				...
				<form id="viewJointHoldersForm" action="account" method="get"
					...
					<div class="form-group">
						<label for="accountNoToView">Account Number:</label> <input
							type="number" id="accountNoToView" name="accountNo" required>
					</div>

					<button type="submit" class="submit-btn">View Joint
						Holders</button>
				</form>
			</div>
		</div>
    ...
    function updateStorageType() {
        const selectedStorage = document.querySelector('input[name="storageType"]:checked');
        if (selectedStorage) {
            const selectedValue = selectedStorage.value;
            const forms = document.querySelectorAll('form');
            
            forms.forEach(form => {
                const storageInput = form.querySelector('input[name="storageType"]');
                if (storageInput) {
                    storageInput.value = selectedValue;
                }
            });
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Transaction tab button listeners
        ...
        // Joint account tab button listeners
    ...
    $(document).ready(function() {
        ...
        // Load user accounts for customers only
        ...
        if ($('#transactions').is(':visible') && userRole === 'CUSTOMER') {
            loadUserAccounts();
        }
        ...
        if (userRole === 'MANAGER' || userRole === 'EMPLOYEE') {
            $('#createAccountForm').on('submit', function(e) {
                ...
                var selectedStorage = $('input[name="storageType"]:checked').val() || 'database';
        ...
        // View Accounts by Branch (staff only)
        if (userRole === 'MANAGER' || userRole === 'EMPLOYEE') {
            ...
            // View Accounts by Customer (staff only)
